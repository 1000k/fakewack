...
export default function PostDetail() {
  ...
  const post: Post = {
    ...
    username: 'username',
    displayName: 'ユーザー名',
    content: 'これは投稿の内容です。写真や動画を共有したり、テキストを投稿したりできます。',
    timeAgo: '2時間前',
    likes: 24,
    comments: 5,
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
          ...
          <div key={comment.id} className="p-4">
            <div className="flex items-start space-x-3">
              <Link href={`/${comment.username}`} className="flex-shrink-0">
                <div className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center">
                  <span className="text-gray-600 font-medium">{comment.displayName.charAt(0)}</span>
                </div>
              </Link>
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <Link href={`/${comment.username}`} className="font-medium hover:underline">
                    {comment.displayName}
                  </Link>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t">
        <form onSubmit={handleCommentSubmit} className="max-w-2xl mx-auto flex items-center p-2">
